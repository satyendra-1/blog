...
import toast from 'react-hot-toast'; // Importing toast for 
const Comments = () => {
  const[comments,setComments]=useState([]);
  const[filter,setFilter]=useState('Not approved')
  // not using dummy data for real data use api call
  const {axios}=useAppContext();
  const  fetchComments=async()=>{
   try{
  const {data}=await axios.get('/api/admin/comments'); // comments api end point
...
<tr>
  <th scope="col" className="px-6 py-3">Blog Title&Comment

  </th>
  <th scope="col" className="px-6 py-3 max-sm:hidden">Date
